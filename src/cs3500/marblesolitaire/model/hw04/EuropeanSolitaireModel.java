...
/**
 * This class is the European equivalent to the solitaire model game.
 */
public class EuropeanSolitaireModel implements MarbleSolitaireModel {
   ...
   *
   * @param sideLengths the arm thickness on the board
   */
  public EuropeanSolitaireModel(int sideLengths) {
    this(sideLengths, (sideLengths * 3 - 3) / 2, (sideLengths * 3 - 3) / 2);
  }

  /**
   * This constructor allows a person to choose an arm thickness and any empty slot on the board.
   *
   * @param sideLengths the arm thickness of the board
   * @param sRow         the row of the empty slot
   * @param sCol         the column of the empty slot.
   */
  public EuropeanSolitaireModel(int sideLengths, int sRow, int sCol) {
    if (sideLengths < 0 || sideLengths % 2 == 0) {
      throw new IllegalArgumentException("Thickness is not a positive odd integer");
    }
  ...
  public boolean isInvalid(int row, int col) {
    return row + col < this.sideLengths - 1
            || row <= col - this.sideLengths * 2 + 1
            || col <= row - this.sideLengths * 2 + 1
            || row + col >= 5 * this.sideLengths - 4;
  }
